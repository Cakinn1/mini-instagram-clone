export const MOCK_DATA = {
  user: {
    id: 0,
    name: "Anthony",
    username: "@ant3071",
    bio: "GOAT",
    avatar: "/assets/profileimage/",
    followers: 100,
    following: 50,
    location: "Australia",
  },
  results: [
    {
      id: 1,
      user: {
        username: "smoothie_afficionado",
        bio: "Crafting delicious and nutritious smoothies. 🍓🥤",
        avatar: "/assets/profileimage",
        followers: 60,
        following: 50,
        location: "Smoothie Bar",
      },
      posts: [
        {
          id: 1,
          photo: "/assets/photos/smoothie.jpeg",
          description:
            "Blended a vibrant smoothie packed with antioxidants and flavor. Cheers to good health! #smoothielove",
          likes: 40,
          bookmarks: 12,
          hashtags: [
            "#smoothieaddict",
            "#healthyliving",
            "#fruitsmoothie",
            "#nutriblast",
            "#wellness",
          ],
          date: "2024-02-16",
          location: "Fruit Fusion Lab",
        },
      ],
    },
    {
      id: 2,
      user: {
        username: "photography_buff_22",
        bio: "Capturing moments that tell a story. 📸🖼️",
        avatar: "/assets/profileimage",
        followers: 110,
        following: 80,
        location: "Photography Studio",
      },
      posts: [
        {
          id: 2,
          photo: "/assets/photos",
          description:
            "A black and white shot that speaks volumes. Sometimes, silence tells the best tales. #photographyart",
          likes: 85,
          bookmarks: 25,
          hashtags: [
            "#blackandwhitephotography",
            "#visualstorytelling",
            "#momentcaptured",
            "#artofphotography",
            "#silentnarrative",
          ],
          date: "2024-02-17",
          location: "Monochrome Chronicles",
        },
      ],
    },
    {
      id: 3,
      user: {
        username: "science_fiction_fan",
        bio: "Immersed in alternate realities and futuristic tales. 🚀📚",
        avatar: "/assets/profileimage",
        followers: 75,
        following: 55,
        location: "Bookstore",
      },
      posts: [
        {
          id: 3,
          photo: "/assets/photos",
          description:
            "Diving into a gripping science fiction novel. Warp speed ahead! #scifireads",
          likes: 48,
          bookmarks: 15,
          hashtags: [
            "#sciencefiction",
            "#bookworm",
            "#readingadventures",
            "#imagineworlds",
            "#bookish",
          ],
          date: "2024-02-18",
          location: "Parallel Realms",
        },
      ],
    },
    {
      id: 4,
      user: {
        username: "hiking_enthusiast_55",
        bio: "Finding solace in the mountains and trails. 🏞️🥾",
        avatar: "/assets/profileimage",
        followers: 90,
        following: 70,
        location: "Mountain Trails",
      },
      posts: [
        {
          id: 4,
          photo: "/assets/photos",
          description:
            "Reached the summit after a challenging hike. The view was worth every step. #mountainmagic",
          likes: 65,
          bookmarks: 22,
          hashtags: [
            "#hikinglife",
            "#summitviews",
            "#trailblazer",
            "#outdoortherapy",
            "#mountaineer",
          ],
          date: "2024-02-19",
          location: "Peak Triumph",
        },
      ],
    },
    {
      id: 5,
      user: {
        username: "coding_enthusiast_101",
        bio: "Turning ideas into reality with lines of code. 💻💡",
        avatar: "/assets/profileimage",
        followers: 100,
        following: 40,
        location: "Home Office",
      },
      posts: [
        {
          id: 5,
          photo: "/assets/photos",
          description:
            "Implemented a groundbreaking feature in my latest project. The joy of coding! #codersjoy",
          likes: 75,
          bookmarks: 20,
          hashtags: [
            "#codingjoy",
            "#programminglife",
            "#techdeveloper",
            "#innovatewithcode",
            "#codersgig",
          ],
          date: "2024-02-20",
          location: "Digital Realm",
        },
      ],
    },
    {
      // id: 6,
      // user: {
      //   username: "fashion_icon_789",
      //   bio: "Redefining style with every outfit. 👗🕶️",
      //   avatar: "/assets/profileimage",
      //   followers: 120,
      //   following: 60,
      //   location: "Fashion Runway",
      // },
      // posts: [
      //   {
      //     id: 6,
      //     user: {
      //       username: "fashion_icon_789",
      //       bio: "Redefining style with every outfit. 👗🕶️",
      //       avatar: "/assets/profileimage",
      //       followers: 120,
      //       following: 60,
      //       location: "Fashion Runway",
      //     },
      //     photo: "/assets/photos",
      //     description:
      //       "Stepping into the spotlight with a bold and chic ensemble. Fashion is self-expression. #styleicon",
      //     likes: 90,
      //     bookmarks: 30,
      //     hashtags: [
      //       "#fashionforward",
      //       "#runwaystyle",
      //       "#trendsetter",
      //       "#fashionista",
      //       "#couture",
      //     ],
      //     date: "2024-02-21",
      //     location: "Couture Carnival",
      //   },
      // ],
    },
    {
      id: 7,
      user: {
        username: "night_sky_observer",
        bio: "Mesmerized by the beauty of the celestial canvas. 🌌🔭",
        avatar: "/assets/profileimage",
        followers: 80,
        following: 50,
        location: "Observatory Deck",
      },
      posts: [
        {
          id: 7,
          photo: "/assets/photos",
          description:
            "Counting stars and identifying constellations. Each twinkling light has a story to tell. #stargazer",
          likes: 55,
          bookmarks: 18,
          hashtags: [
            "#starrynight",
            "#constellationmagic",
            "#nightobsession",
            "#astronomylover",
            "#cosmicwonders",
          ],
          date: "2024-02-22",
          location: "Stellar Nightwatch",
        },
      ],
    },
    {
      id: 8,
      user: {
        username: "gardening_lover_456",
        bio: "Nurturing plants and cultivating green sanctuaries. 🌿🌺",
        avatar: "/assets/profileimage",
        followers: 65,
        following: 55,
        location: "Home Garden",
      },
      posts: [
        {
          id: 8,
          photo: "/assets/photos",
          description:
            "Admiring the blooms in my garden. Nature's beauty in every petal. #greenthumb",
          likes: 42,
          bookmarks: 12,
          hashtags: [
            "#gardenlove",
            "#plantlife",
            "#bloomingbeauty",
            "#gardener",
            "#botanicalbliss",
          ],
          date: "2024-02-23",
          location: "Floral Haven",
        },
      ],
    },
    {
      id: 9,
      user: {
        username: "motorsport_enthusiast",
        bio: "Thriving on the adrenaline of the racetrack. 🏎️🏁",
        avatar: "/assets/profileimage",
        followers: 110,
        following: 80,
        location: "Race Circuit",
      },
      posts: [
        {
          id: 9,
          photo: "/assets/photos",
          description:
            "Accelerated through the turns, embracing the speed and intensity of the race. #speeddemon",
          likes: 75,
          bookmarks: 25,
          hashtags: [
            "#racinglife",
            "#speedfreak",
            "#adrenalinejunkie",
            "#motorsportmania",
            "#racetrackthrills",
          ],
          date: "2024-02-24",
          location: "Fast Lane Adventure",
        },
      ],
    },
    {
      id: 10,
      user: {
        username: "philosophy_mind_123",
        bio: "Contemplating life's mysteries and pondering philosophical thoughts. 🤔📖",
        avatar: "/assets/profileimage",
        followers: 85,
        following: 60,
        location: "Thoughtful Retreat",
      },
      posts: [
        {
          id: 10,
          photo: "/assets/photos",
          description:
            "Engaged in deep philosophical musings. The quest for understanding continues. #philosophy",
          likes: 55,
          bookmarks: 18,
          hashtags: [
            "#philosophicalmind",
            "#wisdomseeker",
            "#existentialism",
            "#deepthoughts",
            "#mindfulpondering",
          ],
          date: "2024-02-25",
          location: "Contemplation Corner",
        },
      ],
    },
    {
      id: 11,
      user: {
        username: "fitness_junkie_789",
        bio: "Pushing physical limits and embracing a healthy lifestyle. 💪🥗",
        avatar: "/assets/profileimage",
        followers: 95,
        following: 70,
        location: "Fitness Haven",
      },
      posts: [
        {
          id: 11,
          photo: "/assets/photos",
          description:
            "Completed an intense workout, feeling the rush of endorphins. Fitness is my therapy. #fitlife",
          likes: 65,
          bookmarks: 22,
          hashtags: [
            "#fitnessjourney",
            "#workoutmotivation",
            "#healthyliving",
            "#fitnessaddict",
            "#sweatitout",
          ],
          date: "2024-02-26",
          location: "Sweat and Shine Studio",
        },
      ],
    },
    {
      id: 12,
      user: {
        username: "coffee_craze_456",
        bio: "Savoring the world one coffee cup at a time. ☕️🌎",
        avatar: "/assets/profileimage",
        followers: 80,
        following: 50,
        location: "Café Culture",
      },
      posts: [
        {
          id: 12,
          photo: "/assets/photos",
          description:
            "Indulged in the rich aroma of freshly brewed coffee. Each sip tells a story of comfort. #coffeelove",
          likes: 50,
          bookmarks: 18,
          hashtags: [
            "#coffeeholic",
            "#coffeeculture",
            "#baristalife",
            "#cozycafes",
            "#muglife",
          ],
          date: "2024-02-27",
          location: "Coffee Bliss Spot",
        },
      ],
    },

    {
      id: 13,
      user: {
        username: "explorer_jane",
        bio: "Adventuring and documenting the wonders of nature. 🌲📷",
        avatar: "/assets/profileimage",
        followers: 120,
        following: 90,
        location: "Nature Trails",
      },
      posts: [
        {
          id: 13,
          photo: "/assets/photos",
          description:
            "Captured the beauty of a secluded waterfall during my latest hike. Nature's marvels never cease to amaze. 🏞️ #naturephotography",
          likes: 65,
          bookmarks: 22,
          hashtags: [
            "#naturelover",
            "#hikingadventures",
            "#exploreoutdoors",
            "#waterfall",
            "#serenity",
          ],
          date: "2024-02-01",
          location: "Hidden Waterfall",
        },
      ],
    },
    {
      id: 14,
      user: {
        username: "gourmet_cuisine",
        bio: "Passionate about creating exquisite culinary delights. 🍽️👨‍🍳",
        avatar: "/assets/profileimage",
        followers: 80,
        following: 70,
        location: "Gourmet Kitchen",
      },
      posts: [
        {
          id: 14,
          photo: "/assets/photos",
          description:
            "Crafted a delectable three-course meal that tantalizes the taste buds. Culinary art at its finest! #foodieheaven",
          likes: 55,
          bookmarks: 18,
          hashtags: [
            "#culinarymasterpiece",
            "#gourmet",
            "#foodporn",
            "#cheflife",
            "#foodgasm",
          ],
          date: "2024-02-03",
          location: "Home Kitchen",
        },
      ],
    },
    {
      id: 15,
      user: {
        username: "coding_genius",
        bio: "Building innovative solutions through code. 💻🚀",
        avatar: "/assets/profileimage",
        followers: 150,
        following: 40,
        location: "Tech Lab",
      },
      posts: [
        {
          id: 15,
          photo: "/assets/photos",
          description:
            "Cracked a complex coding challenge today! Celebrating small victories in the world of programming. #coderslife",
          likes: 90,
          bookmarks: 30,
          hashtags: [
            "#programming",
            "#codinglife",
            "#innovation",
            "#techgeek",
            "#developer",
          ],
          date: "2024-02-05",
          location: "Coding Milestone",
        },
      ],
    },
    {
      id: 16,
      user: {
        username: "yoga_soul",
        bio: "Nurturing mind, body, and soul through the practice of yoga. 🧘‍♀️🌸",
        avatar: "/assets/profileimage",
        followers: 65,
        following: 55,
        location: "Yoga Studio",
      },
      posts: [
        {
          id: 16,
          photo: "/assets/photos",
          description:
            "Found tranquility in a serene yoga session. Embracing the journey of self-discovery. #yogalife",
          likes: 42,
          bookmarks: 12,
          hashtags: [
            "#yogajourney",
            "#mindfulness",
            "#yogainspiration",
            "#selfcare",
            "#serenity",
          ],
          date: "2024-02-02",
          location: "Inner Peace Sanctuary",
        },
      ],
    },
    {
      id: 17,
      user: {
        username: "furry_friends",
        bio: "Dedicated to the well-being of our beloved pets. 🐾❤️",
        avatar: "/assets/profileimage",
        followers: 110,
        following: 65,
        location: "Pet Clinic",
      },
      posts: [
        {
          id: 17,
          photo: "/assets/photos",
          description:
            "Provided care and love to adorable pets at the clinic today. Every pawprint counts! #animalcare",
          likes: 75,
          bookmarks: 20,
          hashtags: [
            "#petcare",
            "#animallove",
            "#veterinary",
            "#pawprints",
            "#furryfriends",
          ],
          date: "2024-02-04",
          location: "Veterinary Clinic",
        },
      ],
    },
    {
      id: 18,
      user: {
        username: "skydiver_extreme",
        bio: "Chasing thrills and conquering the skies. 🪂✈️",
        avatar: "/assets/profileimage",
        followers: 95,
        following: 50,
        location: "Skydiving Drop Zone",
      },
      posts: [
        {
          id: 18,
          photo: "/assets/photos",
          description:
            "Freefalling from the sky was an exhilarating experience! Adrenaline rush like no other. #skydivingadventures",
          likes: 60,
          bookmarks: 15,
          hashtags: [
            "#skydiving",
            "#adrenalinerush",
            "#extremesports",
            "#skyhigh",
            "#thrillseeker",
          ],
          date: "2024-02-06",
          location: "Jumping Into the Blue",
        },
      ],
    },
    {
      id: 19,
      user: {
        username: "motivational_speaker",
        bio: "Empowering others to reach their full potential. 🌟🎤",
        avatar: "/assets/profileimage",
        followers: 200,
        following: 30,
        location: "Inspiration Stage",
      },
      posts: [
        {
          id: 19,
          photo: "/assets/photos",
          description:
            "Spoke at an event today, inspiring others to believe in their capabilities. Your journey matters! #motivationalspeaker",
          likes: 120,
          bookmarks: 35,
          hashtags: [
            "#motivation",
            "#inspiration",
            "#empowerment",
            "#successmindset",
            "#speaker",
          ],
          date: "2024-02-07",
          location: "Empowerment Talk",
        },
      ],
    },
    {
      id: 20,
      user: {
        username: "stargazer_obsession",
        bio: "Lost in the beauty of the cosmos. 🌌🔭",
        avatar: "/assets/profileimage",
        followers: 75,
        following: 60,
        location: "Observatory",
      },
      posts: [
        {
          id: 20,
          photo: "/assets/photos",
          description:
            "Gazing at the stars and pondering the vastness of the universe. Infinite beauty above us. #stargazing",
          likes: 50,
          bookmarks: 18,
          hashtags: [
            "#cosmos",
            "#astronomy",
            "#starrynight",
            "#celestialbeauty",
            "#spaceexploration",
          ],
          date: "2024-02-08",
          location: "Starry Night Observation",
        },
      ],
    },
    {
      id: 21,
      user: {
        username: "coastal_adventurer",
        bio: "Exploring the serene beauty of coastal landscapes. 🏖️🌊",
        avatar: "/assets/profileimage",
        followers: 90,
        following: 70,
        location: "Coastal Exploration",
      },
      posts: [
        {
          id: 21,
          photo: "/assets/photos",
          description:
            "Walking along the shore, feeling the gentle breeze and listening to the ocean's melody. #coastallife",
          likes: 65,
          bookmarks: 22,
          hashtags: [
            "#oceanvibes",
            "#coastalwalk",
            "#beachcombing",
            "#seaside",
            "#naturelovers",
          ],
          date: "2024-02-09",
          location: "Seaside Serenity",
        },
      ],
    },
    {
      id: 22,
      user: {
        username: "artistic_soul_123",
        bio: "Expressing emotions through vibrant strokes. 🎨✨",
        avatar: "/assets/profileimage",
        followers: 85,
        following: 60,
        location: "Art Studio",
      },
      posts: [
        {
          id: 22,
          photo: "/assets/photos",
          description:
            "Finished a new painting today, letting colors tell a story of emotions. #artisticexpression",
          likes: 55,
          bookmarks: 18,
          hashtags: [
            "#arttherapy",
            "#creativeexpression",
            "#painting",
            "#colorful",
            "#artlovers",
          ],
          date: "2024-02-10",
          location: "Canvas of Emotions",
        },
      ],
    },
    {
      id: 23,
      user: {
        username: "sunrise_chaser",
        bio: "Welcoming each day with the first light. ☀️🌄",
        avatar: "/assets/profileimage",
        followers: 110,
        following: 80,
        location: "Scenic Overlook",
      },
      posts: [
        {
          id: 23,
          photo: "/assets/photos",
          description:
            "Witnessed a breathtaking sunrise, a reminder of the beauty in new beginnings. #sunrisechasers",
          likes: 75,
          bookmarks: 25,
          hashtags: [
            "#sunriselovers",
            "#morningglow",
            "#newbeginnings",
            "#natureinspiration",
            "#skyline",
          ],
          date: "2024-02-11",
          location: "Horizon Awakening",
        },
      ],
    },

    {
      id: 24,
      user: {
        username: "science_enthusiast_456",
        bio: "Exploring the wonders of science and discovery. 🔬🚀",
        avatar: "/assets/profileimage",
        followers: 130,
        following: 40,
        location: "Science Lab",
      },
      posts: [
        {
          id: 24,
          photo: "/assets/photos",
          description:
            "Engaged in fascinating experiments today, unraveling the mysteries of the universe. #sciencelover",
          likes: 85,
          bookmarks: 30,
          hashtags: [
            "#science",
            "#experimentation",
            "#discovery",
            "#scientificexploration",
            "#curiosity",
          ],
          date: "2024-02-12",
          location: "Laboratory Discoveries",
        },
      ],
    },
    {
      id: 25,
      user: {
        username: "meditation_guide",
        bio: "Guiding others on the path of inner peace. 🧘‍♂️🌸",
        avatar: "/assets/profileimage",
        followers: 70,
        following: 50,
        location: "Meditation Retreat",
      },
      posts: [
        {
          id: 25,
          photo: "/assets/photos",
          description:
            "Led a meditation session today, helping others find tranquility within. #mindfulness",
          likes: 45,
          bookmarks: 15,
          hashtags: [
            "#meditationjourney",
            "#mindfulnessteacher",
            "#innerpeace",
            "#spiritualgrowth",
            "#calmness",
          ],
          date: "2024-02-13",
          location: "Silent Retreat",
        },
      ],
    },
    {
      id: 26,
      user: {
        username: "guitar_strummer",
        bio: "Expressing emotions through soulful melodies. 🎸🎶",
        avatar: "/assets/profileimage",
        followers: 95,
        following: 60,
        location: "Acoustic Sanctuary",
      },
      posts: [
        {
          id: 26,
          photo: "/assets/photos",
          description:
            "Strummed the guitar and let the music flow. A melody for the heart and soul. #musicaljourney",
          likes: 60,
          bookmarks: 20,
          hashtags: [
            "#guitarist",
            "#acousticmusic",
            "#soulfulmelodies",
            "#musicislife",
            "#harmony",
          ],
          date: "2024-02-14",
          location: "Acoustic Harmony",
        },
      ],
    },
  ],
};

export function fetchData() {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve(MOCK_DATA);
    }, 2000);
  });
}
